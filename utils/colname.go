...
func CountCol(node, bcname string) string {
	return fmt.Sprintf("count_%s", MD5(node, bcname))
}

// block
func BlockCol(node, bcname string) string {
	return fmt.Sprintf("block_%s", MD5(node, bcname))
}

// tx
func TxCol(node, bcname string) string {
	return fmt.Sprintf("tx_%s", MD5(node, bcname))
}

// account
func AccountCol(node, bcname string) string {
	return fmt.Sprintf("account_%s", MD5(node, bcname))
}
